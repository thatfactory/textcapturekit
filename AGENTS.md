...
TextCaptureKit wraps Apple's Vision text-recognition APIs for host-supplied still images. Read [README.md](README.md) and the DocC catalog before changing public behavior.

Keep camera capture, scanner UI, parsing, persistence, and product presentation outside the package. Preserve recognition candidates and uncertainty for host-owned correction.
...
|---|---|
| Package sources | `Sources/TextCaptureKit/` |
| DocC catalog | `Sources/TextCaptureKit/TextCaptureKit.docc/` |
| Unit tests | `Tests/TextCaptureKitTests/` |

## Stack

- Swift 6.4 and Xcode 27.
- iOS 26, macOS 26, tvOS 26, and visionOS 26.
- Swift package CI runner labels: `self-hosted`, `macOS`.
- Run `AgentGuidelines/Scripts/swift_format.sh format-and-lint Package.swift Sources Tests`, `swift test`, and `AgentGuidelines/Scripts/validate_consumer_setup.swift` before handoff.
